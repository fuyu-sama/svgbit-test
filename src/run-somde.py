...
result = run_func.run_somde(count_df, coor_df)
result.to_csv(Path.joinpath(WORKDIR, f"results/somde/{sample}-somde.csv"))
...
for sample in samples:
    ...
    result = run_func.run_somde(count_df, coor_df)
    result.to_csv(
        Path.joinpath(WORKDIR, f"results/somde/DLPFC-{sample}-somde.csv"))
